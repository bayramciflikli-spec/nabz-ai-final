...
type SettingsSection = "account" | "admin" | "language" | "privacy";
...
export default function SettingsPage() {
  ...
  const navItems: { id: SettingsSection; label: string; icon: React.ReactNode }[] = [
    { id: "account", label: "Hesap bilgisi", icon: <User size={18} /> },
    { id: "admin", label: "Admin durumu", icon: <Shield size={18} /> },
    { id: "language", label: "Dil", icon: <Globe size={18} /> },
    { id: "privacy", label: "Gizlilik ve silme", icon: <Trash2 size={18} /> },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-[#0F0F0F] text-white">
      ...
      <div className="flex-1 sm:ml-56 flex flex-col md:flex-row min-h-screen">
        {/* YouTube tarzı sol ayar menüsü */}
        <aside className="w-full md:w-56 shrink-0 border-b md:border-b-0 md:border-r border-white/10 bg-[#0a0a0a]/50">
          <div className="sticky top-0 p-4">
            <div className="flex items-center gap-2 mb-4 px-2">
              <Settings size={20} className="text-white/70" />
              <h1 className="text-lg font-bold">{t("settings.title")}</h1>
            </div>
            <nav className="flex md:flex-col gap-0 overflow-x-auto md:overflow-x-visible py-2 md:py-0">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => setActiveSection(item.id)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-sm font-medium whitespace-nowrap transition-colors ${
        ...
        <main className="flex-1 p-6 md:p-8 max-w-2xl overflow-auto">
        {activeSection === "account" && !user && (
          <p className="text-gray-400 text-sm">Ayarları görüntülemek için giriş yapın.</p>
        )}
        {(activeSection === "account" || !activeSection) && user && (
        ...
        {/* Gizlilik ve silme */}
        {user && activeSection === "privacy" && (
          <section className="mb-8">
            <h2 className="text-lg font-bold mb-3 flex items-center gap-2">
              <Trash2 size={20} className="text-red-400" />
              Silinme Hakkı (GDPR Madde 17)
            </h2>
